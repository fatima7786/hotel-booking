...
import React, {Link,location, useNavigate} from "react-router-dom";
...
const Navbar = () => {
    ...
    return (
            ...
            <nav className={`fixed top-0 left-0 bg-indigo-500 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                <button onClick={opensignIn} className="bg black text-white px-8 py-2.5
            ...
            {user && <button className="border px-4 py-1 text-sm font-light rounded-full cursor-pointer transition-all"onClick={()=>navigate
                    ...
                    {user && <button onClick={opensignIn} className="bg black text-white px-8 py-2.5
                rounded-full ml-4 transition all duration-500">
